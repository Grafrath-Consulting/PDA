...
**Block content conversion**: `lib/blocks/content-html.ts` owns both directions of the MCP content boundary — `toStorageHtml()` on the way in (called once, inside the `save.ts` helpers) and `htmlToPlainText()` on the way out (the `text` field every read tool returns). The format is Markdown in, Markdown out: emphasis, strikethrough, inline and fenced code, bullet and numbered lists (nested by indent), block quotes, links, section breaks and pipe tables. Input is parsed per block, not per document, so prose, tables, lists and raw HTML coexist in one payload. HTML is accepted but is not the authoring format — it exists because storage is HTML, existing cards are HTML, and `update_scratchpad` re-feeds stored HTML through `toStorageHtml()`. `<u>` and `<mark>` are the exception: Markdown has no spelling for them.

Two deliberate departures from CommonMark, both because these notes carry code: `_` is never emphasis (so `DB_NAME` is safe), and `*` emphasis must sit on a word boundary (so `A*B*C` is left alone). A `<` is only treated as markup when it starts a recognised tag, which is what keeps `<<VLSMOTHR>>` and `DHDDTI'<DU86733I` from being eaten by DOMPurify.

Emitted HTML stays inside the TipTap schema — table cells and list items wrap text in `<p>` because TableCell and ListItem take block content, and headings become bold paragraphs because StarterKit runs with `heading: false` and would otherwise drop them. The two functions are inverses: write → read → write is a fixed point, which is what lets a caller read a card, edit it and write it back without formatting decaying. Changing either one means re-checking that property in both directions. `toStorageHtml()` is also idempotent on its own output, which `update_scratchpad` relies on when appending to stored HTML.

The format is documented to callers in two places, both in `lib/mcp/server.ts` and both needing to change together with the parser: `INSTRUCTIONS` (passed as MCP server instructions, which clients surface before any tool call) and `CONTENT_FORMAT` (the per-field description, for clients that show tool schemas but not instructions).
